# -*- coding: utf-8 -*-
"""
Created on Thu Nov 27 10:40:29 2025
...
# -*- coding: utf-8 -*-
"""
Created on Thu Oct  9 14:57:08 2025
...
def run_scenario(
    ...
    CO2Cost_L2_2 = quicksum(
        new_loc_CO2[c] * quicksum(f2_2[c, d, mo] for d in Dcs for mo in Modes)
        * (co2_cost_per_ton_New / 1000)    # convert kg→ton and multiply by €/ton
        for c in New_Locs
    )
    ...
    # OBJECTIVE
    # -----------------------------
    model.setObjective( Sourcing_L1 + Handling_L2 + Handling_L3 + LastMile_Cost + CO2_Mfg + Total_Transport+ Total_InvCost_Model+ CO2Cost_L2_2+ Cost_NewLocs,
        GRB.MINIMIZE
    )
...
def simulate_scenarios_full():
    ...
    for level in demand_levels:
        print(f"\n🚀 Running SC2F for {int(level*100)}% demand...\n")
